# `createDirectedMatrices` function
library(data.table)
library(reshape)

# Description -------------------------------------------------------------
# This function is called by the main one, `ReadMoveBankDataFormat`. It loops on all time groups. For each one, it updates the numbers of co-occurring vultures, including self. Also updates the counters of simultaneously-tracked vultures. Note that each dyad, including self, is counted twice. So A--B and B--A, or if self then A--A and A--A.
# NOTE: takes ~3.5 hours to run!

# The function ------------------------------------------------------------
createDirectedMatrices <- function(Dataset, DistThreshold, 
                                   sim = SimlDataPntCnt, co = CoocurCountr){
